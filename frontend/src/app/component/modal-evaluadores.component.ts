    ...
    <div class="modal-ev-body">
      ...
      <div class="ev-list" *ngIf="!cargando">
        <div class="ev-item" *ngFor="let e of asignados">
          ...
          <button class="btn-quitar" *ngIf="!e.es_dueno" (click)="quitar(e)" title="Remover evaluador">
...
export class ModalEvaluadoresComponent implements OnInit, OnChanges {
  ...
  cargar(): void {
    ...
    this.http.get<EvaluadorInfo[]>(`${this.API}/${this.idProceso}?tipo=${this.tipo}`).subscribe({
      next: (data) => { this.asignados = data; this.cargarDisponibles(); },
      error: () => { this.cargando = false; }
    });
  }

  cargarDisponibles(): void {
    this.http.get<EvaluadorInfo[]>(`${this.API}/${this.idProceso}/disponibles?tipo=${this.tipo}`).subscribe({
      next: (data) => { this.disponibles = data; this.cargando = false; },
      error: () => { this.cargando = false; }
    });
  }
  ...
  asignar(e: EvaluadorInfo): void {
    this.http.post(`${this.API}/${this.idProceso}`, {
      ...
      next: () => this.cargar(),
      error: (err) => alert(err?.error?.mensaje || 'Error al asignar.')
    });
  }

  quitar(e: EvaluadorInfo): void {
    if (!confirm(`¿Remover a ${e.nombre_completo} como evaluador?`)) return;
    this.http.delete(`${this.API}/${this.idProceso}/${e.id_usuario}?tipo=${this.tipo}`).subscribe({
      next: () => this.cargar(),
      error: (err) => alert(err?.error?.mensaje || 'Error al remover.')
    });
  }
